                   ...
                   size_t                    size)
{
    ...
    int mode = patch.GetAdcValue(patch.B8)
    /** The first control will set the reverb time */
    float rev_time = 0.3 + (0.67 * patch.GetAdcValue(patch.CV_3));
    ...
    float       damp_control = patch.GetAdcValue(patch.CV_4);
    float       damping
   ...
   * dry amount and the effects send amount. */
    ...
    float inlevel = patch.GetAdcValue(patch.CV_1);

    /** The fourth controll will control how much of the input is sent to the
   * reverb */
    float sendamt = patch.GetAdcValue(patch.CV_2);
